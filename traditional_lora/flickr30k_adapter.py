...
本模块实现了Flickr30K数据集与COCO数据集接口的适配：
- 读取本地Flickr30K数据（图片 + 标注文件）
- 接口兼容COCOCaptionDataset
- 复用COCO的复杂度计算和分层逻辑

数据集路径要求：
/root/autodl-tmp/flickr30k/
├── flickr30k-images/              # 图片目录
│   ├── 1000092795.jpg
│   ├── 10002456.jpg
│   └── ...
└── flickr_annotations_30k.csv     # CSV标注文件（包含raw, split, filename等列）
