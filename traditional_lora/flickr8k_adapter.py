"""
Flickr8k 数据集适配器（Traditional LoRA 版本）

本文件是上级目录 `flickr8k_adapter.Flickr8kAdapter` 的轻量复制版本，
目的是在 `traditional_lora` 子项目中提供与 `COCODatasetConfig` 兼容的
Flickr8k 适配器，使 Traditional LoRA 训练 / 评估 / 测试可以直接使用
Flickr8k 数据集。
"""
...
class Flickr8kAdapter:
    ...
    def __init__(self, data_root: str = "/root/autodl-tmp/Flickr8k"):
        ...
        logger.info(f"[Traditional LoRA] Flickr8k 适配器初始化完成，根目录: {data_root}")
    ...
    def validate_paths(self) -> bool:
        ...
        if missing:
            logger.error(f"[Traditional LoRA] Flickr8k 缺失关键路径: {missing}")
            return False
        ...
        logger.info("[Traditional LoRA] Flickr8k 数据集路径验证完成")
        return True
    ...
    def _load_split_lists(self) -> Dict[str, List[str]]:
        ...
        for split, list_path in split_to_file.items():
            if not os.path.exists(list_path):
                logger.warning(f"[Traditional LoRA] Flickr8k {split} 划分文件不存在: {list_path}")
                continue
            try:
                with open(list_path, "r", encoding="utf-8") as f:
                    filenames = [line.strip() for line in f if line.strip()]
                split_to_filenames[split] = filenames
                logger.info(f"[Traditional LoRA] Flickr8k {split} 划分图片数: {len(filenames)}")
            except Exception as e:
                logger.error(f"[Traditional LoRA] 读取 Flickr8k {split} 划分文件失败: {e}")
    ...
    def _load_tokens(self) -> Dict[str, List[str]]:
        ...
        if not os.path.exists(self.tokens_file):
            logger.error(f"[Traditional LoRA] Flickr8k 标注文件不存在: {self.tokens_file}")
            return filename_to_captions

        logger.info(f"[Traditional LoRA] 正在解析 Flickr8k 标注文件: {self.tokens_file}")
        try:
            ...
            logger.error(f"[Traditional LoRA] 解析 Flickr8k 标注文件失败: {e}")

        logger.info(f"[Traditional LoRA] 共解析到 {len(filename_to_captions)} 张图片的描述")
        return filename_to_captions
    ...
    def _need_regenerate(self, json_file: str) -> bool:
        ...
        try:
            ...
            if "category_id" not in anns[0]:
                logger.warning(
                    f"[Traditional LoRA] 旧格式 Flickr8k COCO Caption（缺少 category_id），重新生成: {json_file}"
                )
                return True
            logger.info(f"[Traditional LoRA] Flickr8k COCO Caption 已存在且格式正确: {json_file}")
            return False
        except Exception as e:
            logger.warning(f"[Traditional LoRA] 读取 Flickr8k COCO Caption 失败，将重新生成: {e}")
            return True

    def _generate_single_split(self, split: str, dst_file: str) -> None:
        filenames = self.split_to_filenames.get(split, [])
        if not filenames:
            logger.warning(f"[Traditional LoRA] Flickr8k {split} 划分为空，跳过生成: {dst_file}")
            return
        ...
        if not images or not annotations:
            logger.warning(f"[Traditional LoRA] Flickr8k {split} 无有效数据，跳过写入: {dst_file}")
            return
        ...
        try:
            ...
            logger.info(
                f"[Traditional LoRA] ✓ 生成 Flickr8k {split} COCO Caption: "
                f"{dst_file} ({len(images)} 张图片, {len(annotations)} 条描述)"
            )
        except Exception as e:
            logger.error(f"[Traditional LoRA] 写入 Flickr8k {split} COCO Caption 失败: {e}")

    def _generate_coco_format_annotations(self) -> None:
        logger.info("[Traditional LoRA] 正在生成 Flickr8k COCO Caption 标注...")
        split_cfgs = [
        ...
        logger.info("[Traditional LoRA] Flickr8k COCO Caption 生成/校验完成")
